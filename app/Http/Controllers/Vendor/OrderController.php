...
class OrderController extends Controller
{
    ...
    public function orderView($id)
    {
        ...
        $order = Order::with('customers')->where('id',$id)->where('vendor_id',$vendorId)->get();

        $orderDetails = OrderDetails::with('products')->where('order_id', $id)->where('vendor_id', $vendorId)->get();
        $shipping = Shipping::where('order_id',$id)->first();

        return view('vendor.pages.order.pages.OrderView',['order'=> $order,'orderDetails' => $orderDetails, 'shipping' => $shipping]);
    }
